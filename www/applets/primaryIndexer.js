function refreshPrimaryIndexerResult(data){
    deactivateLoading();
    var finalResult='';
    const userSearch = document.getElementById('primaryIndexerSearchBarText').value.toUpperCase();
    // split result.sequence into 3 parts using userSearch as the delimiter
    document.getElementById('primaryIndexerCount').innerHTML = data.length;
    document.getElementById('primaryIndexerSeq').innerHTML = userSearch;
    ...
    document.getElementById("primaryIndexerEntries").innerHTML = finalResult;

}

function primaryIndexerSearch(){
    activateLoading();
    const value = document.getElementById('primaryIndexerSearchBarText').value;
    network.tx({action:'indexBySeq', parameters:{seq:value}});
}